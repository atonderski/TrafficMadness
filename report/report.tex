...
\twocolumn[
  \begin{@twocolumnfalse}
    \maketitle
    \begin{abstract}
        \noindent Traffic congestions are a widely recognized problem. Even though how and why they form hs been studied extensively no real solution has been implemented. Expansion of the road network has been shown to have little to no effect. Earlier work has suggested that introduction of autonomous vehicles and increased inter-vehicle communication will have a positive effect on the prevention and ability to dissipate congestions. This paper presents a simple agent-based model used to study the formation of congestions and the effect of improved velocity adaptations mimicing the capabilities of self-driving cars.

        In the model we find that traffic density has a large impact on congestion, influencing both spontaneous formation and how systems are effected and recover from artifically induced congestions. Low density traffic allows for higher average flow while at the same time being more resilient to congestion. The velocity adaptation improvements decrease the effect of congestion and allows for more rapid recovery. In conclusion, the model suggests that using inter-vehicle communication or other means to measure and adapt to velocity changes in traffic could lead to decreased congestion, even in dense traffic. Although, the findings are not conclusive and further improvements to the model would need to be done for any implications to be plausiable for real-world scenarios.   
    \end{abstract}
  \end{@twocolumnfalse}
]
...
\subsection{Traffic congestion as a phase transition}
